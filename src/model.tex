...
We differentiate the two types with two different arrows, double arrow ($\to$ \hspace{-1.4em} $\to$ or \texttt{>>}) for \textit{start} rupture points and simple arrow ($\to$ or \texttt{->}) for \textit{post} rupture points.
...
\begin{bnf*}
  ...
  \bnfprod{fn}         {\bnftd{Imperative language with inline~} \bnfpn{stream}}\\
\end{bnf*}
...
Listing \ref{lst:source} presents a simple web application to illustrate the fluxional execution model.
This application reads a file, and sends it back along with a request counter.
The equivalent fluxional code is in listing \ref{lst:fluxional}.
% The execution model is illustrated with an example application whose source code is in listing \ref{lst:source} and fluxional code in listing \ref{lst:fluxional}.
The transformation pocess is explained later, in section \ref{section:compiler}.
The execution cycle of the example is illustrated in figure \ref{fig:MesSys}.

Bigger circles represent registered fluxions, while smaller ones indicate the step in the execution.
...
\nt{The discussion of the execution model is only discussed on a simple example (figure 2) and not discussed in general.}

\nt{In section 2.3, it is said that variable "res" holds the references to the network sockets and it is the variable group network depends on.
This is a bit misleading since in the next paragraph groups are created automatically.
The sentence about the group network makes the reader believe the relation between res and network has to be specified, but it is not explained how.
It is only while ready section 3 that the reader can understand the relation is actually detected during the transformation phase.
However it is still unclear why "res" is supposed to holds its content, and how/where it is specified in the listings (or elsewhere).}
...
The \texttt{handler} function, line \ref{lst:source-handler} to \ref{lst:source-handler-end}, receives the input stream of request.
The \texttt{count} variable at line \ref{lst:source-counter} counts the requests.
This variable needs to be persisted in the fluxion \textit{context}.
...
\nt{Transformation discussed in Sec 2.3 between Listing 1 and Listing 2 is not understandable many parts are not discussed in text.
Even the parts discussed are difficult to follow (eg. "res is created by the first fluxion main..." how is this done ?
res is a shared variable, thus it requires to be protected how is this accomplished?)}
...
The fluxion \texttt{handler} doesn't have any dependencies, hence it can be executed in parallel, and replicated.
...
This variable is created and consumed inside this group, therefore, it doesn't prevent the whole group from being replicated.

However, the last fluxion, \texttt{reply}, depends on its context to hold the variable \texttt{count} which prevents this replication.
If it did not rely on this state, the group \texttt{network} would be stateless, and could be replicated to cope with the incoming traffic.
