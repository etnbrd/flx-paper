...
Such applications are built using functions, but these functions are executed sequentially to assure the exclusivity of access on the global memory.
This is a serious performance issue, as it avoid to leverage the parallelism of modern architectures.

We present in this section a different execution model that isolate the memory of some functions.
...
Due to the nature of real-time web application, we insist on the streaming aspect of this communications.
The execution units exchange streams of messages, that correspond with the input stream of requests on the web application.
