...
We differentiates the two types with two different arrows, double arrow (\texttt{>>}) for \textit{start} rupture points and simple arrow (\texttt{->}) for \textit{post} rupture points.
...
In figure \ref{fig:MesSys}, circles represent registered fluxions.
...
When the application receives a request,it triggers the flow with a \texttt{start} message containing the request, \circled{2}.
