...
A fluxion is composed of a unique name, a processing function, and a local memory called a \textit{context}.
Its function consumes an input stream and generates one or more outputs streams to destination fluxions.
...
A function might need to share its state with other functions.
The context of their encapsulating fluxion need to hold the same state.
Such fluxions are grouped to ensure the dependency on this shared state.
Each fluxion is associated with tags to express these groups.
The message streams hold the data communications ; similarly these groups represent the graph of state communications between fluxion.
...
Each event-loop has its own memory and thread of execution, it is isolated from the others.
The messaging system allows the execution to be distributed on several event-loop.
It carries messages from one event-loop to the other, and assure that the message is queued on the event-loop executing the destination fluxion.
...
This execution model allows to parallelize the execution of different parts of an application, hence it improves the scalability of that application.
As a fluxion contains its state and expresses its dependencies, it can be migrated.
It allows to adapt the number of fluxions per core to get the desired throughput.
