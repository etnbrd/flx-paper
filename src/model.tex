...
However, the last fluxion, \texttt{reply}, depends on its context to holds the variable \texttt{count} which prevents this replication.
If it did not relied on this state, the group \texttt{network} would be stateless, and could be replicated to cope with the incoming traffic.
