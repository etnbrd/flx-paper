...
We develop a compiler that transforms a simple \textit{Node.js} application into a fluxional system compliant with the architecture described in section \ref{section:model}.
...
Our compiler uses a new approach to find rupture points into most of \textit{Node.js} applications, marking out the independent application parts.
...
Figure \ref{fig:basicrp} and \ref{fig:specialrp} illustrate the different types of interface implied by the rupture point between two application parts.
...
This method walk an intermediate representation of the source code to spot the statements modifying a certain variable.
...
The variable tracker is still in progress and is limited to only a few cases.
